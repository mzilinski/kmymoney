...
//                            enter/skip schedule, sectionOrder)
...
//   accountAllocationModel - AccountAllocationModel (per-account base-currency magnitudes)
Kirigami.ScrollablePage {
    ...
    ColumnLayout {
        ...
        // ---- Welcome / empty state (not a reorderable section) ----
        ...
        // ---- Sections, in the order of the classic home-page ItemList (MR-B9) ----
        // homeBridge.sectionOrder carries only the SHOWN sections (hidden = absent).
        // The Repeater is eager and the Loaders stay synchronous (default), so every
        // section keeps live scene-graph nodes under homeContent — required by the
        // B7 print grab. Data self-gates live on the Loader (never read back from
        // item.visible, which reports EFFECTIVE visibility and would latch false).
        Repeater {
            model: homeBridge.sectionOrder
            delegate: Loader {
                required property string modelData
                Layout.fillWidth: true
                visible: homeBridge.fileOpen
                         && (modelData !== "allocation" || accountAllocationModel.count > 0)
                         && (modelData !== "schedules" || schedulesModel.count > 0)
                sourceComponent: modelData === "netWorthHeader" ? netWorthHeaderSection
                               : modelData === "netWorthPie"    ? netWorthPieSection
                               : modelData === "allocation"     ? allocationSection
                               : modelData === "accounts"       ? accountsSection
                               : modelData === "schedules"      ? schedulesSection
                               : null
            }
        }

        // ---- Net-worth header (text) ----
        // Position & visibility come from homeBridge.sectionOrder (classic ItemList
        // code 8 emits the header+pie pair; the classic net-worth graph, code 6, has
        // no QML counterpart).
        Component {
            id: netWorthHeaderSection
        ...
        // Position & visibility come from homeBridge.sectionOrder (second half of the
        // code-8 pair).
        Component {
            id: netWorthPieSection
            Kirigami.AbstractCard {
                contentItem: ColumnLayout {
        ...
        // count is a real property (rowCount is a method); the Loader hides the card
        // when nothing has a non-zero magnitude to draw. QML-only section: it has no
        // classic home-page ItemList code; homeBridge.sectionOrder anchors it at the
        // position of the 8/-8 entry, always-on.
        Component {
        ...
        // Position & visibility come from homeBridge.sectionOrder (classic codes 2|3).
        Component {
            id: accountsSection
        ...
        // method, not bindable); the Loader hides the card when nothing is due.
        // Position & visibility come from homeBridge.sectionOrder (classic code 1).
        Component {
            id: schedulesSection
            Kirigami.AbstractCard {
                contentItem: ColumnLayout {
